...
for module in $MODULES; do
  ...
  while IFS= read -r file; do
    ...
    imports=$(grep -E "^\s*\"github.com/kamil5b/go-pste-monolith/internal/modules" "$file" || true)
    ...
    while IFS= read -r import_line; do
      ...
      imported_module=$(echo "$import_line" | grep -oE 'github.com/kamil5b/go-pste-monolith/internal/modules/[^/]+' | sed 's|.*modules/||')
...
for module in $MODULES; do
  ...
  while IFS= read -r file; do
    ...
    bad_imports=$(grep -E "^\s*\"github.com/kamil5b/go-pste-monolith/internal/(app|infrastructure)" "$file" || true)
